   ...
   for(strucnum=0; strucnum<gMultiCount; strucnum++)
   {
      ...
      for(z1=gZoneList[strucnum]; z1!=NULL; NEXT(z1))
      {
         ...
         else
         {
            ...
            strncpy(z2->chain1,       z1->chain1, 8);
            z2->start1       = z1->start1;
            strncpy(z2->startinsert1, z1->startinsert1, 8);
            z2->stop1        = z1->stop1;
            strncpy(z2->stopinsert1,  z1->stopinsert1, 8);
            strncpy(z2->chain2,       z1->chain2, 8);
            z2->start2       = z1->start2;
            strncpy(z2->startinsert2, z1->startinsert2, 8);
            z2->stop2        = z1->stop2;
            strncpy(z2->stopinsert2,  z1->stopinsert2, 8);
            z2->mode         = z1->mode;
         }
      }
   }
...
structure.\n");
   ...
   */
   if(!gIterate)
   {
      ...
      blMatMult33_33(gRotMat[strucnum],gRotMatTwist,rotmat_repos);
      ...
      blMatMult33_33(rotmat_repos,rotmat_refit,rotmat_final);
             ...
             BOOL ByAtm)
{
   ...
   {
      INIT(gRZoneList[strucnum],ZONE);
      strcpy(gRZoneList[strucnum]->chain1, " ");
      gRZoneList[strucnum]->start1        = -999;
      strcpy(gRZoneList[strucnum]->startinsert1, " ");
      gRZoneList[strucnum]->stop1         = -999;
      strcpy(gRZoneList[strucnum]->stopinsert1, " ");
      strcpy(gRZoneList[strucnum]->chain2, " ");
      gRZoneList[strucnum]->start2        = -999;
      strcpy(gRZoneList[strucnum]->startinsert2, " ");
      gRZoneList[strucnum]->stop2         = -999;
      strcpy(gRZoneList[strucnum]->stopinsert2, " ");
...
***/
      ...
      /* Reference structure                                            */
      /*
        if(!blFindZonePDB(gRefPDB, z->start1, z->startinsert1, 
                        z->stop1, z->stopinsert1, z->chain1, z->mode, 
                        &ref_start, &ref_stop))
      */
      if(!blFindZonePDB(refpdblist, z->start1, z->startinsert1, 
                      z->stop1, z->stopinsert1, z->chain1, z->mode, 
                      &ref_start, &ref_stop))
      {
         char zone1[64],
      ...
      /* Mobile structure                                               */
      if(!blFindZonePDB(gFitPDB[strucnum], z->start2, z->startinsert2, 
...
int CreateFitArrays(int strucnum)
{
   ...
   if(gZoneList[strucnum] == NULL)
   {
      ...
      strcpy(gZoneList[strucnum]->chain1, " ");
      gZoneList[strucnum]->start1       = -999;
      strcpy(gZoneList[strucnum]->startinsert1, " ");
      gZoneList[strucnum]->stop1        = -999;
      strcpy(gZoneList[strucnum]->stopinsert1, " ");
      strcpy(gZoneList[strucnum]->chain2, " ");
      gZoneList[strucnum]->start2       = -999;
      strcpy(gZoneList[strucnum]->startinsert2, " ");
      gZoneList[strucnum]->stop2        = -999;
      strcpy(gZoneList[strucnum]->stopinsert2, " ");
   ...
   for(z=gZoneList[strucnum]; z!=NULL; NEXT(z))
   {
      ...
      /* Mobile structure                                               */
      if(!blFindZonePDB(gMobPDB[strucnum], z->start2, z->startinsert2, 
                   z->stop2, z->stopinsert2, z->chain2, z->mode, 
                   &mob_start, &mob_stop))
      {
         char zone1[64],
...
/*>void CalculateRotationMatrix(REAL RotAngle, REAL Matrix[3][3])
   ...
   12.10.17 Changed to bl() functions
*/
void CalculateRotationMatrix(REAL RotAngle, REAL Matrix[3][3])
{
   /* Convert to radians                                                */
