/*************************************************************************
   ...
   Copyright:  SciTech Software / UCL 1992-2020
...
**************************************************************************
   ...
   V3.1.1 10.04.13 Improved error message when zone not found
   V3.2   11.10.17 Fixed code to work with new BiopLib where chains and
                   inserts are all arrays
   V3.3   27.04.20 Now handles model numbers
...
/*>void logo(void)
   ...
   10.04.13 Version 3.1.1
   11.10.17 Version 3.2
   27.04.20 Version 3.3
*/
void logo(void)
{
   printf("\n                  PPPPP                FFFFFF ii   tt\n");
...
/*>void SetRMSZone(char *command)
   ...
   10.04.13 Improved error message when Zone not found By: ACRM
   11.10.17 New bioplib supports chains and inserts of more than
            one character - so uses null terminated strings instead
            of characters
   12.10.17 Delete zones that are not found
*/
void SetRMSZone(char *command)
{
   int   start1,  stop1,
   ...
   char  chain1[8],  chain2[8],
         startinsert1[8], stopinsert1[8],
         startinsert2[8], stopinsert2[8];
   ZONE  *z        = NULL,
         *prevZone = NULL;
   int   warned    = 0;
      ...
      if(SeqZone > -1)
      {
         ...
         */
         ...
         else
         {
            ...
            strncpy(z->chain1, chain1, 8);
            z->start1              = start1;
            strncpy(z->startinsert1, startinsert1, 8);
            z->stop1               = stop1;
            strncpy(z->stopinsert1,  stopinsert1, 8);

            strncpy(z->chain2,       chain2, 8);
            z->start2              = start2;
            strncpy(z->startinsert2, startinsert2, 8);
            z->stop2               = stop2;
            strncpy(z->stopinsert2,  stopinsert2, 8);

            z->mode         = SeqZone?ZONE_MODE_SEQUENTIAL:gCurrentMode;

            /* CTP: Check for overlap                                   */
            /* 10.04.13 Improved not found error message By: ACRM       */
            /* 12.10.17 Restructured so CheckOverlap() isn't called
                        twice
            */
            overlap = CheckOverlap(z,gRZoneList[strucnum],strucnum);
            if(overlap > 1)
            {
            ...
            {
               printf("   Error: Failed to find new zone - the chain or \
residue was not found.\n");
               /* Delete this zone                                      */
               free(z);
               if(prevZone)
               {
                  /* There was a previous zone so unlink this one       */
                  prevZone->next = NULL;
               }
               else
               {
                  /* No previous zone so set head of list to null       */
                  gRZoneList[strucnum] = NULL;
               }
            }
            else
...
/*>BOOL ReadStructure(int structure, char *filename, int strucnum,
   ...
   WOW! This needs rewriting so that it doesn't contain the same code
        twice!!!
   ...
   13.03.09 Set function to clear fitted coordinates when loading a new
            ...
            coordinates.
   27.04.20 Added handling of model number
*/
                   ...
                   int  xmasFormat)
{
   ...
   strncpy(filename, inFilename, MAXBUFF);
   ...
   {
      printf("   Reading reference structure (%s)...\n", inFilename);
      strcpy(gRefFilename,inFilename);
      ...
      {
         if(gHetAtoms)
            gRefWPDB = ReadWholePDBModel(fp, modelNum);
         else
            gRefWPDB = ReadWholePDBAtomsModel(fp, modelNum);
         ...
         if(gOccRank > 1)
         {
            ...
            if(gHetAtoms)
               gRefWPDB->pdb = ReadPDBOccRankModel(fp,&natoms,gOccRank, modelNum);
            else 
               gRefWPDB->pdb = ReadPDBAtomsOccRankModel(fp,&natoms,gOccRank, modelNum);
         }
...
insertions.\n");
   ...
   {
      printf("   Reading mobile structure (%s)...\n", inFilename);
      strcpy(gMobFilename[strucnum],inFilename);
      ...
      {
         if(gHetAtoms)
            gMobWPDB[strucnum] = ReadWholePDBModel(fp, modelNum);
         else 
            gMobWPDB[strucnum] = ReadWholePDBAtomsModel(fp, modelNum);
         ...
         if(gOccRank > 1)
         {
            ...
            if(gHetAtoms)
               gMobWPDB[strucnum]->pdb = 
                  ReadPDBOccRankModel(fp,&natoms,gOccRank,modelNum);
            else 
               gMobWPDB[strucnum]->pdb = 
                  ReadPDBAtomsOccRankModel(fp,&natoms,gOccRank,modelNum);
         }
...
int DoCommandLoop(FILE *script)
{
   ...
   for(;;)
   {
      ...
      if(script == stdin)
      {
         ...
         /* 07.07.09 Replace strcpy() with strncpy() By: ACRM           */
         strncpy(comline, line_read, MAXBUFF);
         TERMINATE(comline);
      ...
      */
      ...
      /* Main parser                                                    */
      switch(blMparse(comline,NCOMM,gKeyWords,gNumParam,gStrParam,
                      &NParam))
      {
      case PARSE_ERRC:
...
structure when performing\n");
      ...
      if(SeqZone > -1)
      {
         ...
         else
         {
            ...
            /* CTP: Check for overlap                                   */
            /* 10.04.13 Improved not found error message By: ACRM       */
            overlap = CheckOverlap(z,gZoneList[snum],snum);
            if(overlap > 1)
            {
               printf("   Warning: New zone overlaps existing zone.\n");
               gUserFitZone = TRUE;
...
structure when performing\n");
      ...
      while(z!=NULL)
      {
         ...
         if(CHAINMATCH(z->chain1, chain1)              &&
            (z->start1       == start1)                &&
            INSERTMATCH(z->startinsert1, startinsert1) &&
            (z->stop1        == stop1)                 &&
            INSERTMATCH(z->stopinsert1,  stopinsert1)  &&
            CHAINMATCH(z->chain2, chain2)              &&
            (z->start2       == start2)                &&
            INSERTMATCH(z->startinsert2, startinsert2) &&
            (z->stop2        == stop2)                 &&
            INSERTMATCH(z->stopinsert2, stopinsert2)   &&
            (z->mode         == SeqZone ? 
             ZONE_MODE_SEQUENTIAL : gCurrentMode))
         {
...
*/
   if((zone1 == zone2) && retval==0)
   {
      *start2       = *start1;
      *stop2        = *stop1;
      strncpy(chain2, chain1, 8);
      strncpy(startinsert2, startinsert1, 8);
      strncpy(stopinsert2,  stopinsert1,  8);
   }
   else
   {
      dash=FindDash(zone2);
      ...
      if((ptr=strchr(zone2,'*'))!=NULL)
      {
         ...
         *stop2  = -999;
         ...
         /* If * was not the first char in the zone spec., then the first
            ...
            FIXME to work with multi-character chain names
         */
         if(ptr != zone2)
         {
            chain2[0] = zone2[0];
...
***/
   ...
   if(((chains = blGetPDBChainLabels(gRefPDB, &nchains)) != NULL) &&
      ((nchains > 1) || (chains[0][0] != ' ')))
   {
      
      fprintf(fp,"   Reference structure Chains: ");
      for(i=0; i<nchains; i++)
      {
         if(i) fprintf(fp,", ");
         fprintf(fp,"%s", chains[i]);
      }
      fprintf(fp,"\n");
   }
   ...
   if(((chains = blGetPDBChainLabels(gMobPDB[0], &nchains)) != NULL) &&
      ((nchains > 1) || (chains[0][0] != ' ')))
   {
      
      fprintf(fp,"   Mobile structure Chains:    ");
      for(i=0; i<nchains; i++)
      {
         if(i) fprintf(fp,", ");
         fprintf(fp,"%s", chains[i]);
                ...
                int stop,  char *stopinsert)
{
   ...
   if((start == (-999)) && (stop  == (-999)))
   {
      if(*chain == ' ')
         ...
         sprintf(zone,"Chain %s",chain);
   }
   else
   {
      if(start==(-999))
      {
         sprintf(part1,"%s*",    chain);
         sprintf(part2,"%s%d%s", chain, stop, stopinsert);
      }
      else if(stop==(-999))
      {
         sprintf(part1,"%s%d%s", chain, start, startinsert);
         sprintf(part2,"%s*",    chain);
      }
      else
      {
         sprintf(part1,"%s%d%s", chain, start, startinsert);
...
written.\n");
   ...
   else
   {
      if((fp=blOpenOrPipe(filename))==NULL)
      {
         printf("   Error==> Enable to open file for writing (%s)\n",
                filename);
      }
      else
      {
         ...
         /* TODO: The 0 should be the number of TER records            */
         if(gReadHeader)
...
/*>void WriteMulti(char *ext)
   ...
   27.04.20 Now handles model numbers
*/
void WriteMulti(char *ext)
{
   char filename[MAXBUFF*2],
        tmpFilename[MAXBUFF+40];
   int  i,
        j,
        modelNum;
   ...
   for(i=0; i<gMultiCount; i++)
   {
      strncpy(tmpFilename, gMobFilename[i], MAXBUFF);
      /* Work from the end of string to strip off the extension         */
      j = strlen(tmpFilename)-1;

      modelNum = GetModelNum(tmpFilename);
      
      /* Step back until we find a . delimiting the extension           */
      while((tmpFilename[j] != '.') && (j>=0))
      {
      ...
      if((strlen(tmpFilename) + strlen(ext) + 5) >= MAXBUFF+40)
      {
      ...
      strncpy(filename, tmpFilename, MAXBUFF);

      /* Append the model number if not 1                               */
      if(modelNum != 1)
         sprintf(filename, "%s%c%d", tmpFilename, MODEL_SEP, modelNum);
...
notation.\n");
   ...
   for(strucnum=0; strucnum<gMultiCount; strucnum++)
   {
      SeqZone = ParseZone(zone_command, 
                          &start1, &stop1, chain1, 
                          startinsert1, stopinsert1, 
                          &start2, &stop2, chain2,
                          startinsert2, stopinsert2,
...
int ConvertResidueToSequential(ZONE *input_zone, int strucnum)
{
   ...
   for(p=gRefPDB; p!=NULL; NEXT(p))
   {
      ...
      if((p->resnum    != prev_residue) ||
         !INSERTMATCH(p->insert, prev_insert)  ||
         !CHAINMATCH(p->chain,   prev_chain))
      {
         residue_count++;
      }
      ...
      if((CHAINMATCH(input_zone->chain1, " ") || 
          CHAINMATCH(input_zone->chain1, p->chain)) &&
      ...
      if((CHAINMATCH(input_zone->chain1, " ") || 
         ...
         INSERTMATCH(input_zone->startinsert1, p->insert) &&
         (output_zone->start1 == 0))
      {
         output_zone->start1 = residue_count;
      }
      ...
      if((CHAINMATCH(input_zone->chain1, " ") || 
         ...
         INSERTMATCH(input_zone->stopinsert1, p->insert) &&
         (output_zone->stop1 == 0))
      {
         output_zone->stop1 = residue_count;
      }
      ...
      strncpy(prev_insert, p->insert, 8);
   ...
   for(p=gMobPDB[strucnum]; p!=NULL; NEXT(p))
   {
      ...
      /* Check for Start                                                */
      /* Start residue number == -999                                   */
      if((CHAINMATCH(input_zone->chain2, " ") || 
          CHAINMATCH(input_zone->chain2, p->chain)) &&
      ...
      if((CHAINMATCH(input_zone->chain2, " ") || 
         ...
         INSERTMATCH(input_zone->startinsert2, p->insert) &&
         (output_zone->start2 == 0))
      {
         output_zone->start2 = residue_count;
      }
      ...
      if((CHAINMATCH(input_zone->chain2, " ") || 
         ...
         INSERTMATCH(input_zone->stopinsert2, p->insert) &&
         (output_zone->stop2 == 0))
      {
         output_zone->stop2 = residue_count;
      }
   ...
   strcpy(input_zone->chain1, " ");
   strcpy(input_zone->chain2, " ");
...
int CheckOverlap(ZONE *inputtest, ZONE *inputlist, int strucnum)
{
  ...
  INIT(zonelist,ZONE);
  strncpy(zonelist->chain1,       inputtest->chain1, 8);
  zonelist->start1              = inputtest->start1;
  strncpy(zonelist->startinsert1, inputtest->startinsert1, 8);
  zonelist->stop1               = inputtest->stop1;
  strncpy(zonelist->stopinsert1,  inputtest->stopinsert1, 8);
  strncpy(zonelist->chain2,       inputtest->chain2, 8);
  zonelist->start2              = inputtest->start2;
  strncpy(zonelist->startinsert2, inputtest->startinsert2, 8);
  zonelist->stop2               = inputtest->stop2;
  strncpy(zonelist->stopinsert2,  inputtest->stopinsert2, 8);
  ...
  {
     strncpy(zb->chain1,        zc->chain1, 8);
     zb->start1               = zc->start1;
     strncpy(zb->startinsert1,  zc->startinsert1, 8);
     zb->stop1                = zc->stop1;
     strncpy(zb->stopinsert1,   zc->stopinsert1, 8);
     strncpy(zb->chain2,        zc->chain2, 8);
     zb->start2               = zc->start2;
     strncpy(zb->startinsert2,  zc->startinsert2, 8);
     zb->stop2                = zc->stop2;
     strncpy(zb->stopinsert2,   zc->stopinsert2, 8);
     zb->mode                 = zc->mode;
         ...
         for(; (pdbref!= NULL) && (pdbmob != NULL); NEXT(pdbref))
         {
            ...
            if(((int)(pdbref->bval) == label) && !ref_assigned)
            {
               if(ref_only)
               {
                  ...
                  strcpy(z->chain1,       " ");
                  z->start1             = ref_res;
                  strcpy(z->startinsert1, " ");
                  z->stop1              = ref_res;
                  strcpy(z->stopinsert1,  " ");
                  strcpy(z->chain2,       " ");
                  z->start2             = ref_res;
                  strcpy(z->startinsert2, " ");
                  z->stop2              = ref_res;
                  strcpy(z->stopinsert2, " ");
...
int ConvertSequentialToResidue(ZONE *input_zone, int strucnum)
{
   ...
   z = output_zonelist;
   strncpy(z->chain1,        p->chain, 8);
   z->start1       = p->resnum;
   strncpy(z->startinsert1,  p->insert, 8);
   z->stop1        = p->resnum;
   strncpy(z->stopinsert1,   p->insert, 8);
   strncpy(z->chain2,        q->chain, 8);
   z->start2       = q->resnum;
   strncpy(z->startinsert2,  p->insert, 8);
   z->stop2        = q->resnum;
   strncpy(z->stopinsert2,   q->insert, 8);
   z->mode         = ZONE_MODE_RESNUM;
   z->next         = NULL;
   ...
   {
      /* Skip to Next Ref                                               */
      while(CHAINMATCH(p->chain, z->chain1) && 
            (p->resnum    == z->stop1)  &&
            INSERTMATCH(p->insert, z->stopinsert1))
      {
         NEXT(p);
      }
      
      /* Skip to Next Mob                                               */
      while(CHAINMATCH(q->chain, z->chain2) && 
            (q->resnum    == z->stop2)  &&
            INSERTMATCH(q->insert, z->stopinsert2))
      {
         NEXT(q);
      }
      
      /* Check Prev Chain                                               */
      if(!CHAINMATCH(p->chain, z->chain1) ||
         !CHAINMATCH(q->chain, z->chain2))
      {
         /* Add New Zone                                                */
         ALLOCNEXT(z,ZONE);
   ...
   /* Set Start of List                                                 */
   strncpy(input_zone->chain1,       output_zonelist->chain1, 8);
   input_zone->start1              = output_zonelist->start1;
   strncpy(input_zone->startinsert1, output_zonelist->startinsert1, 8);
   input_zone->stop1               = output_zonelist->stop1;
   strncpy(input_zone->stopinsert1,  output_zonelist->stopinsert1, 8);
   strncpy(input_zone->chain2,       output_zonelist->chain2, 8);
   input_zone->start2              = output_zonelist->start2;
   strncpy(input_zone->startinsert2, output_zonelist->startinsert2, 8);
   input_zone->stop2               = output_zonelist->stop2;
   strncpy(input_zone->stopinsert2,  output_zonelist->stopinsert2, 8);
...
ZONE *ChainList(PDB *pdblist)
{
   ...
   /* Set Chain ID                                                      */
   strncpy(chainlist->chain1, pdblist->chain, 8);
   strncpy(prev_chain,        pdblist->chain, 8);
   ...
   for(z = chainlist, p=pdblist; p!=NULL; NEXT(p))
   {
      ...
      if(!CHAINMATCH(p->chain, prev_chain))
      {
            ...
            for(z=gZoneList[strucnum]; z!=NULL; NEXT(z))
            {
                   ...
                   (z->mode ==  ZONE_MODE_SEQUENTIAL)))
               {
                  if(!found)
                  {
                     ...
                     strncpy(chainid, mob->chain1, 8);
                     found = TRUE;
                  }
                  else
                  {
                     /* Return FALSE if different chain ID found        */
                     if(!CHAINMATCH(mob->chain1, chainid))
                     {
                        OneToOne = FALSE;
                        goto Return;
                     }
...
int EnforceOneToOneChains(int strucnum)
{
   ...
   {
      best = 0;
      strncpy(ref->chain2, chainlist_mob->chain1, 8);
      ...
      for(mob=chainlist_mob; mob!=NULL; NEXT(mob))
      {
         ...
         {
            best       = curr;
            strncpy(ref->chain2, mob->chain1, 8);
   ...
   {
      best = 0;
      strncpy(mob->chain2, chainlist_ref->chain1, 8);
      ...
      for(ref=chainlist_ref; ref!=NULL; NEXT(ref))
      {
         ...
         {
            best       = curr;
            strncpy(mob->chain2, ref->chain1, 8);
...
structure!\n");
   ...
   for(i=0;i<gMultiCount && gUserFitZone;i++)
   {
      ...
      {         
         strncpy(za->chain1,       zb->chain2, 8);
         za->start1              = zb->start2;
         strncpy(za->startinsert1, zb->startinsert2, 8);
         za->stop1               = zb->stop2;
         strncpy(za->stopinsert1,  zb->stopinsert2, 8);
      }
   ...
   for(za = InputA; za != NULL; NEXT(za))
   {
      for(zb = InputB; zb != NULL; NEXT(zb))
      {
             ...
             zb->mode == ZONE_MODE_SEQUENTIAL))
         {
            ...
            /* Set Everything Else                                      */
            strcpy(zo->startinsert1, " ");
            strcpy(zo->startinsert2, " ");
            strcpy(zo->stopinsert1,  " ");
            strcpy(zo->stopinsert2,  " ");
            strcpy(zo->chain1,       " ");
            strcpy(zo->chain2,       " ");
   ...
   for(zi = InputZone; zi != NULL; NEXT(zi))
   {
      for(zo = OverlapZone; zo != NULL; NEXT(zo))
      {
             ...
             zo->mode == ZONE_MODE_SEQUENTIAL))
         {
            ...
            /* Set other values                                         */
            strcpy(zn->startinsert1, " ");
            strcpy(zn->startinsert2, " ");
            strcpy(zn->stopinsert1,  " ");
            strcpy(zn->stopinsert2,  " ");
            strcpy(zn->chain1,       " ");
            strcpy(zn->chain2,       " ");
...
/************************************************************************/
/*>WHOLEPDB *ReadWholePDBAtomsModel(FILE *fpin, int modelNum)
   ----------------------------------------------------------
   Reads the specified model in a PDB file (ATOMs only)

   Should move into BiopLib

   27.04.20 Original   By: ACRM
*/
WHOLEPDB *ReadWholePDBAtomsModel(FILE *fpin, int modelNum)
{
   WHOLEPDB *wpdb;
   wpdb = blDoReadPDB(fpin, FALSE, 1, modelNum, TRUE);
...
/************************************************************************/
/*>WHOLEPDB *ReadWholePDBModel(FILE *fpin, int modelNum)
   -----------------------------------------------------
   Reads the specified model in a PDB file (ATOM and HETATMs)
...
{
   WHOLEPDB *wpdb;
   wpdb = blDoReadPDB(fpin, TRUE, 1, modelNum, TRUE);
   wpdb->pdb = blRemoveAlternates(wpdb->pdb);
   return(wpdb);
}
...
/************************************************************************/
/*>PDB *ReadPDBOccRankModel(FILE *fp, int *natom, int OccRank, 
                            int modelNum)
   -----------------------------------------------------------
   Reads the specified model and occupancy in a PDB file (ATOMs and 
   HETATMs)
   ...
   27.04.20 Original   By: ACRM
*/
PDB *ReadPDBOccRankModel(FILE *fp, int *natom, int OccRank, int modelNum)
{
   PDB *pdb = NULL;
   WHOLEPDB *wpdb;
   *natom=(-1);

   if((wpdb = blDoReadPDB(fp, TRUE, OccRank, modelNum, FALSE))!=NULL)
   {
...
/************************************************************************/
/*>PDB *ReadPDBAtomsOccRankModel(FILE *fp, int *natom, int OccRank, 
                                 int modelNum)
   ----------------------------------------------------------------
   Reads the specified model and occupancy in a PDB file (ATOMs only)
                              ...
                              int modelNum)
{
   ...
   *natom=(-1);
   ...
   if((wpdb = blDoReadPDB(fp, FALSE, OccRank, modelNum, FALSE))!=NULL)
   {
      blFreeStringList(wpdb->header);
      blFreeStringList(wpdb->trailer);
      *natom = wpdb->natoms;
...
/************************************************************************/
/*>int GetModelNum(char *filename)
   -------------------------------

   Takes a filename with an optional model seperator (MODEL_SEP) and
   integer number following the model seperator (e.g. pdb1abc.ent%2
   pdb1abc.ent%10). This number is taken as the PDB MODEL number. The
   routine returns the model number (1 if not specified) and truncates
   the filename at the model seperator if found.
...
int GetModelNum(char *filename)
{
   ...
   /* Look for the last modelSep and if not found return 1              */
   if((modelSep = strrchr(filename, MODEL_SEP))==NULL)
      return(1);

   /* Step through characters after the modelSep. If any are non-numeric
      return 1
   */
   for(chp=modelSep+1; *chp != '\0'; chp++)
   {
      if(!isdigit(*chp))
      {
         return(1);
   ...
   /* Terminate at the modelSep                                         */
   *modelSep = '\0';

   return(atoi(modelSep+1));
}
