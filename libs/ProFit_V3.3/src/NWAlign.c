/*************************************************************************
   ...
   Copyright:  SciTech Software 1992-2020
...
/*>NWAlign(int strucnum)
   ...
   16.07.08 Changed alignment function from align() to affinealign()
            ...
            By: CTP
   27.04.20 Removed unused variable aj  By: ACRM
*/
...
alignment\n");
   ...
   /*
   score = blAlign(gRefSeq, ref_len, gMobSeq[strucnum], mob_len, FALSE, 
   ...
   */
   ...
   offset = 0;
   printf("   ");
   for(i=0,ai=0; ai<align_len; ai++)    /* Prints ref sequence     */
   {
      char  buffer[8];
...
/*>int AlignmentFromZones(BOOL fasta)
   ...
   10.09.08 Ensured sequentially numbered fitting zones had breaks
            between chains.
   14.01.09 Added output to file.
   27.04.20 Doubled chainids[] buffer size  By: ACRM
*/
         ...
         for(mob = chainlist[1]; mob!=NULL; NEXT(mob))
         {
            for(z=gZoneList[i]; z!=NULL; NEXT(z))             
            {
                  ...
                  (z->mode == ZONE_MODE_SEQUENTIAL))
               {
                  ...
                  sprintf(chainids[0],"%s Chain '%s'",ids[0],ref->chain1);
                  sprintf(chainids[1],"%s Chain '%s'",ids[1],mob->chain1);
...
/*>void AlignZone(ZONE *alignzone, int strucnum, BOOL append)
   ...
   Note: ref_chain and mob_chain seem to be redundant
...
int AlignmentWrapper(int strucnum, char *command, BOOL append)
{
   ...
   else 
   {
      ...
      char chain1[8], startinsert1[8], stopinsert1[8],
           chain2[8], startinsert2[8], stopinsert2[8];
...
multiple\n");
      ...
      INIT(alignzone, ZONE);
      strncpy(alignzone->chain1,       chain1, 8);
      alignzone->start1       = start1;       
      strncpy(alignzone->startinsert1, startinsert1, 8); 
      alignzone->stop1        = stop1;        
      strncpy(alignzone->stopinsert1,  stopinsert1, 8);  
      strncpy(alignzone->chain2,       chain2, 8);       
      alignzone->start2       = start2;       
      strncpy(alignzone->startinsert2, startinsert2, 8); 
      alignzone->stop2        = stop2;        
      strncpy(alignzone->stopinsert2,  stopinsert2, 8);  
