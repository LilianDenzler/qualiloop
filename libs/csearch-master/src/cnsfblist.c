...
residue %.4s %.4s\n", pstruct.resnme[desc->resno-1],pstruct.resid[desc->resno-1]);
   ...
   {
      for (ires=desc->resno+1; ires <= clsres; ires++)
      {
         if (ires != desc->resno+1)
         {
            if (!(catom[nat++] = typeinres(ires,
               ires+1==segfirst ? "NT  " : "N   "))) break;
      ...
      {
         ok = false;
         fprintf(out,"Error in cnsfblist -- \
