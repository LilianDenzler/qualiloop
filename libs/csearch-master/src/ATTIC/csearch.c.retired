/* 08.04.95 Fixed to call clear_sc()     By: ACRM
   24.05.94 Does memory allocation for restart_st    By: ACRM
*/
...
void csearch( int argc, char **argv)
{
   UBYTE eof = FALSE;
   ...
   /* ACRM 24.05.94
      Allocate memory for restart information (was in pscgncmnd.c)
   */
   ...
   while(!eof && !feof(in))
   {
      ...
      case do_restart:
         ...
         cg.restart_stlen = (int)strlen(cg.restart_st);
         break;
      case do_debug:
         debug();
         break;
...
/* OMUPD rkw 08/10/93 Added sidechain clear routine */
      case do_sclear:
         if (!pdb_fp)
         {
            fprintf(out,"Warning: PDB must be specified first\n");
         ...
         padspace(g_strparam[1],4);
         padspace(g_strparam[2],4);
/* ACRM Corrected to clear_sc()
         clear_side(g_strparam[1],g_strparam[2]);
*/
...
/* OMUPD end */
   ...
   else
   {
      if (RunCGEN)
      {
         fprintf(fp_suc, "CONGEN finished normally, producing %d", confnum);
         (confnum == 1) ? fprintf(fp_suc, " conformation\n"):
                          fprintf(fp_suc, " conformations\n");
      }
      else
      {
         fprintf(fp_suc, "CONGEN finished normally\n");
      }
      fclose(fp_suc);
   }
}
