...
/* The main command parser in main() has hit a CGEN command, so
   ...
   24.05.94 Removed allocation of cg.restart_st (Now in csearch.c)
*/
...
/*         p->desc = (struct backbone_d *) alloc(sizeof(struct backbone_d)); */
         p->desc = (dof_descriptor *) alloc(sizeof(struct backbone_d));
...
/*         p->desc = (struct sidechain_d *) alloc(sizeof(struct sidechain_d)); */
         p->desc = (dof_descriptor *) alloc(sizeof(struct sidechain_d));
...
/*         p->desc = (struct status_d *) alloc(sizeof(struct status_d)); */
         p->desc = (dof_descriptor *) alloc(sizeof(struct status_d));
         p->dof_type = status_t;
         break;
      case do_loops:
         fprintf(out,"Now Processing Degree of Freedom command \
...
/*         p->desc = (struct rbest_d *) alloc(sizeof(struct rbest_d)); */
         p->desc = (dof_descriptor *) alloc(sizeof(struct rbest_d));
         p->dof_type = rbest_t;
         break;
...
/*         p->desc = (struct evaluate_d *) alloc(sizeof(struct evaluate_d)); */
         p->desc = (dof_descriptor *) alloc(sizeof(struct evaluate_d));
